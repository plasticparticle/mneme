...
import { ENTRY_DAYS, ENTRIES, findJournal, type Entry } from '../data/sample';
...
function EntryRow({ e, onOpen, compact }: { e: Entry; onOpen: (e: Entry) => void; compact?: boolean }): VNode {
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {e.media > 0 && <Icon name="image" size={14} color="var(--ink-3)" />}
        </div>
...
function Heatmap({ weeks = 17 }: { weeks?: number }): VNode {
  ...
  for (let i = 0; i < weeks * 7; i++) {
    ...
    const lvl = seed > 8 ? 3 : seed > 6 ? 2 : seed > 3 ? 1 : 0;
    cells.push(lvl);
  }
...
function Cell({ d, today, selected, onSelect, big }: { d: number; today: number; selected: number; onSelect: (d: number) => void; big?: boolean }): VNode {
  const entries = ENTRY_DAYS[d] || [];
...
export function CalendarScreen({ desk, onOpenEntry }: { desk: boolean; onOpenEntry: (e: Entry) => void }): VNode {
  const [offset, setOffset] = useState(0); // months from June 2026
  const baseY = 2026;
  const baseM = 5; // June
  ...
  const dayEntries = isJune ? ENTRY_DAYS[selected] || [] : [];
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: big ? 6 : 2 }}>
        ...
        {Array.from({ length: meta.days }).map((_, i) => (
          <Cell key={i} d={i + 1} today={isJune ? today : -1} selected={isJune ? selected : -1} onSelect={setSelected} big={big} />
        ))}
      </div>
    </div>
  );
  ...
  const MonthNav = ({ big }: { big?: boolean }): VNode => (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        {!isJune && <Btn kind="quiet" size="sm" onClick={() => { setOffset(0); setSelected(9); }}>Today</Btn>}
      ...
      <div style={{ height: '100%', display: 'flex', background: 'var(--paper)' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '24px 28px', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 18 }}>
            ...
            <Stat n={isJune ? '8' : '0'} label="entries" />
        ...
        <div style={{ width: 340, borderLeft: '1px solid var(--line)', display: 'flex', flexDirection: 'column', background: 'var(--surface-2)' }}>
          <div style={{ padding: '24px 22px 14px' }}>
            <div style={{ fontFamily: 'var(--ui)', fontSize: 12, fontWeight: 700, letterSpacing: 0.6, textTransform: 'uppercase', color: 'var(--ink-3)' }}>
              {isJune ? `${WD[(new Date(year, month, selected).getDay() + 6) % 7]} · ${MONTHS[month]} ${selected}` : 'No selection'}
            </div>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '0 18px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {dayEntries.length ? dayEntries.map((e) => <EntryRow key={e.id} e={e} onOpen={onOpenEntry} compact />) : <EmptyDay onNew={() => onOpenEntry(ENTRIES[0])} />}
          </div>
    ...
    <div style={{ height: '100%', overflow: 'auto', background: 'var(--paper)', paddingBottom: 84 }}>
      <div style={{ padding: '60px 18px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 10, margin: '16px 0 14px' }}>
          ...
          <Stat n={isJune ? '8' : '0'} label="entries" boxed />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '22px 2px 12px' }}>
          <h3 style={{ fontFamily: 'var(--serif)', fontSize: 19, fontWeight: 500, color: 'var(--ink)', margin: 0 }}>{isJune ? `${MONTHS[month]} ${selected}` : MONTHS[month]}</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {dayEntries.length ? dayEntries.map((e) => <EntryRow key={e.id} e={e} onOpen={onOpenEntry} />) : <EmptyDay onNew={() => onOpenEntry(ENTRIES[0])} />}
        </div>
      </div>
    </div>
  );
