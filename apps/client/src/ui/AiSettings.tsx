    ...
    >
      ...
      >
        ...
        <h3 style={{ fontFamily: 'var(--serif)', fontSize: 19, fontWeight: 500, color: 'var(--ink)', margin: '0 0 14px', display: 'flex', alignItems: 'center', gap: 9 }}>
          <Icon name="feather" size={18} color="var(--accent)" /> AI assistant
        </h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '12px 14px', borderRadius: 12, background: 'var(--paper)', border: '1px solid var(--line)', cursor: 'pointer', textAlign: 'left' }}
          >
            <div>
              <div style={{ fontFamily: 'var(--ui)', fontSize: 14, fontWeight: 600, color: 'var(--ink)' }}>Enable AI features</div>
              <div style={{ ...pStyle, fontSize: 12 }}>Ask questions about your journal, get writing help.</div>
            </div>
          ...
          {card(
            'ollama',
            'Ollama',
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: 0.4, textTransform: 'uppercase', color: 'var(--accent-ink)', background: 'var(--accent-soft)', border: '1px solid var(--accent-line)', borderRadius: 6, padding: '2px 7px' }}>stays on this device</span>,
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <p style={pStyle}>Your entries are processed by a model running on your own machine. Nothing leaves this device.</p>
              <div>
              ...
              <div>
                ...
                {models.length > 0 ? (
                  ...
                  <input style={inputStyle} value={form.ollama.model} onInput={(e) => patch({ ollama: { ...form.ollama, model: (e.target as HTMLInputElement).value } })} placeholder="llama3.2 — or Test connection to list" />
                )}
              </div>
              <p style={{ ...pStyle, fontSize: 11.5, color: 'var(--ink-3)' }}>
                Accessing Ollama from a non-localhost origin needs <span style={{ fontFamily: 'var(--mono)' }}>OLLAMA_ORIGINS</span> set on the Ollama side; Safari may block localhost from an https page.
              </p>
            </div>,
          )}
          ...
          {card(
            'anthropic',
            'Anthropic',
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: 0.4, textTransform: 'uppercase', color: 'var(--ink-3)', background: 'var(--paper)', border: '1px solid var(--line)', borderRadius: 6, padding: '2px 7px' }}>cloud</span>,
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '11px 13px', borderRadius: 12, background: 'var(--accent-soft)', border: '1px solid var(--accent-line)', fontFamily: 'var(--ui)', fontSize: 12.5, lineHeight: 1.5, color: 'var(--accent-ink)' }}>
                ...
                <span>
                  When you use AI features, the entries used as context are sent, <strong>decrypted</strong>, to Anthropic over
                  HTTPS — leaving Mneme's end-to-end encryption for that request. Your relay never sees them. Anthropic's API
                  terms apply.
                </span>
              </div>
              <div>
          ...
          {form.enabled && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <Btn kind="ghost" size="sm" onClick={() => void runTest()}>{test.state === 'busy' ? 'Testing…' : 'Test connection'}</Btn>
              {test.state === 'ok' && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: 'var(--ui)', fontSize: 12.5, color: 'var(--accent-ink)' }}><Icon name="check" size={14} color="var(--accent)" /> Connected</span>}
              {test.state === 'fail' && <span style={{ fontFamily: 'var(--ui)', fontSize: 12.5, color: 'var(--ink-2)' }}>{test.message}</span>}
            </div>
          )}

          {error && <p style={{ ...pStyle, color: 'var(--accent-ink)' }}>Could not save: {error}</p>}
